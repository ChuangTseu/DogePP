...
struct IncludedScopeInfo {
	...
	void MergeFromMovedIncludedScopeInfo(IncludedScopeInfo&& movedScopeInfo, int aboveScopeLineStart)
	{
		...
		sonsDelim.push_back({ movedScopeInfo.vLinesSubLocs.front().m_start, movedScopeInfo.vLinesSubLocs.back().m_end });
...
struct StrCTX {
	StrCTX(std::string& capturedString, std::string strName) : 
		StrCTX(capturedString, capturedString.cbegin(), strName) {}

	StrCTX(std::string& capturedString, StrCItT customHeadIt, std::string strName) : 
		m_baseStr(capturedString), m_head(customHeadIt), m_name(strName) {
...
#define TMP_DEFAULT_REGISTERED_POS_BEHAVIOR e_insidereplacedzonebehaviour_GOTO_START
	...
	void AdjustScopeInfoLocations(SubStrLoc replacedSubLoc, StrSizeT filledLength)
	{
		...
		{ // Pop out lines (typically from an unselected #if/#else guard zone)
			...
			DOGE_DEBUG_ASSERT(std::distance(lbOnEndIt, lbOnStartIt) >= 0 && std::distance(lbOnEndIt, lbOnStartIt) < 2);
			if (std::distance(lbOnEndIt, lbOnStartIt) == 1)
			{
			...
			for (size_t i = poppedBegPos; i < vLinesSubLoc.size(); ++i)
			{
				if (vLinesSubLoc[i].m_end <= replacedSubLoc.m_end)
					vLinesSubLoc[i].m_start = vLinesSubLoc[i].m_end = replacedSubLoc.m_start + filledLength;
				else
